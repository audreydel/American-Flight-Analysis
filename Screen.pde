...
class Screen {
 ...
 // BarChart screenChart2;
  ...
  Screen(color backgroundColor, ArrayList widgies, ArrayList widgie2, ArrayList taskBar)
  {
    this.widge=widgies;
    this.widge2=widgie2;
  ...
  void draw()
  {
    ...
    if (currentScreen==screen3) //piechart
    {
      ...
      for(int i = 0; i < taskBar.size(); i++){
        Widget aWidget = (Widget) taskBar.get(i);
        aWidget.draw();
      }
    }
  ...
  void mousePressed() {
    ...
    for (int i = 0; i<taskBar.size(); i ++) {
      Widget taskBar2 = (Widget) taskBar.get(i);
      int  event3 =taskBar2.getEvent (mouseX,mouseY);
      switch (event3)
      {
        case PIECHART2RIGHT:
          currentScreen = screen5;
